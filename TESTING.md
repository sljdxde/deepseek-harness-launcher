...
14. 会话完成通知：由启动器自己启动 dsh 后，在 Harness 里让任一主会话完成一轮回复，菜单栏图标应在数秒内出现红色数字角标，菜单顶部列出该会话；点击具体条目后复用或打开 Harness，并直接定位到对应会话且角标清零；“清除完成提醒”同样清零；复用外部 Harness（未加载内置 patch）时至少打开 Harness 首页，不报错、不崩溃。
15. 标签页复用：Chrome/Safari 中同时打开 Harness 和其他标签页时，菜单「打开 Deepseek Harness」、全局快捷键、完成提醒点击都应通过 lsof/ps 检测到已有页面，并在首次授权“允许启动器控制浏览器”后选中 Harness 标签，不调用 `NSWorkspace.open` 创建重复标签页；完成提醒还应定位到对应会话。没有任何浏览器连着时才新开；拒绝自动化授权、外部实例或不支持的浏览器时至少前置浏览器/打开页面，不报错、不崩溃。连接在 Chrome 等浏览器的 Helper 子进程名下时，也能沿父子链找到主进程。
